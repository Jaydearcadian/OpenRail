...
Last updated: 2026-06-20 (frontend Stream redesign session)
...
1. No Nonce Lane architecture yet.
   - No onchain `accountNonceTracks[payer][nonceChannel]`.
   - No `nonceChannel`.
   - No `nonceValue`.
   - No SDK `NonceEngine`.
   - Current nonce safety is object/payload scoped, mainly RailsCard `SealedVault.nonce` and SDK envelope nonce.
2. Public writes are not exposed in the web app or CLI.
   - Protocol and SDK write paths exist.
...
4. The formal product Receipt Layer is not implemented yet.
   - No canonical `metadataHash` field.
   - No product receipt JSON schema.
   - No separate residual recovery receipt API.
   - No receipt PDF, QR, or export layer.
   - No access credential binding to product receipt ID and metadata hash.
   - Existing receipts primarily mean terminal onchain `SettlementReceipt` records.
5. V2 Vault, Conduit, and DOF architecture is still conceptual.
