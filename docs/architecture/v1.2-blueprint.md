...
**On-chain foundation: IMPLEMENTED** (branch `v1-2-nonce-lanes`, local `sui move test` green — not yet published).

- `move/sources/nonce_account.move` — `NonceAccount { payer, lanes: Table<u64,u64> }`, a per-payer shared object. `create_nonce_account`, `verify_and_consume(account, payer, nonce_channel, nonce_value)` (asserts `next == value` then increments; aborts → whole tx reverts, so failed/ replayed opens never advance a lane), `next_nonce(account, channel)`.
- Nonce-lane consumption wired into both payer-initiated opens: `mint_and_fund_envelope` (RailsFlow/direct) and `create_sealed_vault` (RailsCard). `payer = sender`, so the lane is consumed atomically against the caller.
- **`metadataHash` binding**: `Paycard<T>` and `SealedVault<T>` gained a `metadata_hash: vector<u8>` field; threaded through `new_paycard`; the vault signature message (`build_vault_message`) now appends `nonce_channel` + `metadata_hash`; a new `events::ChannelMetadataAnchored` event is emitted at mint. `SettlementReceipt` is unchanged (indexer/SDK receipt parsing stays stable).
- `PROTOCOL_VERSION = 12` constant + `protocol_version()` view.
- Tests: `move/tests/nonce_account_tests.move` (consume/replay-abort/wrong-payer/independent-lanes) + updated `paycard_tests` / `sealed_vault_tests` (metadata bound, duplicate-nonce rejected, all lifecycle suites green). 24/24 pass.
- Operator publish: `scripts/publish-v1-2.sh`.

**This is a new V1.2 package cut** — the signed vault message and open-entry signatures changed. The deployed V1.1 package + current SDK stay valid for existing proofs. **Required next (Phase 2, NOT done):** update the SDK `buildVaultMessage` + vault/mint params to the new ABI, add the `NonceEngine`, product-receipt builders, and CLI writes; then publish and repoint `services/receipt-api`, `apps/web`, `sdk` to the new package ID.

**SDK + CLI: IMPLEMENTED** (branch `v1-2-nonce-lanes`) — `buildVaultMessage` + PTB builders synced to the new ABI, `NonceEngine` (`sdk/src/nonce.ts`), CLI write commands (`nonce-create`/`open`/`open-vault`/`unseal`/`claim`/`cancel`/`resolve`), and the operator runbook `scripts/testnet-e2e-v1-2.md`.

**Product receipt layer: SDK IMPLEMENTED** — `sdk/src/product-receipt.ts`: canonical `computeMetadataHash`/`metadataHashHex`/`verifyMetadataHash` (SHA-256 over domain-separated canonical JSON; the same hash bound on-chain at open), `ProductMetadataV1`/`ProductReceiptV1` types, and `createPaymentReceipt`/`createSettlementReceipt`/`createResidualRecoveryReceipt` with a deterministic `receiptId`. Worker exposes `GET /v1/nonces/:nonceAccountId/:lane` (devInspect of `next_nonce`). Pending: PDF/QR/export, a Worker product-receipt route (needs off-chain metadata sourcing), `ChannelMetadataAnchored` indexing, and the by-`:payer` nonce form (needs a `NonceAccountCreated` Move event).

**Public write UX (web): IMPLEMENTED** (branch `web-wallet-writes`) — @mysten/dapp-kit + @mysten/enoki (Google/Facebook/Twitch zkLogin + sponsored gas), `useChannelWrite` state machine, "Open a rail" surface (open/claim/cancel/resolve). Live e2e is operator-run (publish + Enoki/Google config).

**Access credentials: IMPLEMENTED** (branch `v1-2-access-credentials`) — `sdk/src/access-credential.ts`: payer-signed `AccessCredentialV1` over domain-separated canonical claims (merchant co-sign optional), `Authorization: OpenRails` header helpers, `verifyAccessCredentialSignature` + `verifyAccessCredential` (signature → payer-matches-address → expiry → channel active via `sdk/src/channel-state.ts` `getChannelState` or the proof API). Worker `POST /v1/access/verify` hosts the check; CLI `credential issue` / `verify`. Pending: on-chain `metadataHash` cross-check (needs `ChannelMetadataAnchored` indexing), revocation, a demo gated-resource.

V1.2 is now feature-complete in code across `v1-2-nonce-lanes` / `web-wallet-writes` / `v1-2-access-credentials`; remaining work is operator verification (publish + live e2e) and the small deferrals noted above.
